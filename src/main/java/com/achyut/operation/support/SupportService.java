...
public class SupportService {
    ...
    private final WarrantyCoveragePolicy warrantyCoveragePolicy;
    ...
    public WarrantyView createWarranty(WarrantyRequest r){
        ...
        audit.record("ASSET",asset.getId(),"WARRANTY_CREATED",actor(r.actor()),r.startDate()+" to "+r.endDate());
        return view(w);
    }

    public ComplaintView createComplaint(ComplaintRequest r){
        Asset asset=assets.findById(r.assetId()).orElseThrow(()->new NoSuchElementException("Asset not found"));
        boolean covered=warrantyCoveragePolicy.isCovered(asset,LocalDate.now());
        Complaint c=complaints.save(Complaint.builder().complaintNumber(numbers.next("CMP")).asset(asset).description(r.description()).photoUrl(r.photoUrl())
            .warrantyCovered(covered).status(Complaint.Status.OPEN).build());
        audit.record("ASSET",asset.getId(),"COMPLAINT_OPENED",actor(r.actor()),c.getComplaintNumber()+" warrantyCovered="+covered);
        return view(c);
    }
    ...
    public ComplaintView updateComplaint(Long id, ComplaintUpdate r){
        ...
        audit.record("ASSET",c.getAsset().getId(),"COMPLAINT_STATUS_CHANGED",actor(r.actor()),c.getComplaintNumber()+" "+old+" -> "+r.status());
        return view(c);
    }
